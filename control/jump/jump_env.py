...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.ref_motion.load(cur_path + '/jump.skmo')
        self.ref_motion.reset_root_trajectory_jump(self.ref_skel)
        ...
        # draw box
        self.box_height = 0.001
        self.box_size = [1.5, self.box_height, 0.5]

        self.world.skeletons[0].add_box("box", self.box_size, [0.9, 0.9, 0.9])
        self.world.skeletons[0].body("box").set_collidable(False)

        self.box_pos = []
        self.box_pos.append(np.array([0., 0.5 * self.box_height, 2.]))
        ...
        self.feet_contact_end_frame = 34
        self.feet_contact_start_frame = 49
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            if json_file == json_path+"jump_000000000074_keypoints.json":
                for i in range(15):
                    self.up_angle_list.append(up_angle)
    ...
    def reward(self):
        ...
        if self.feet_contact_start_frame-2 <= self.current_frame:
        ...
        parabola_hint_ranges = [(self.feet_contact_end_frame, self.feet_contact_start_frame, ['h_heel_left', 'h_heel_right'], 0)]
        ...
        for parabola_hint_range in parabola_hint_ranges:
            ...
            if parabola_hint_range[0] < self.current_frame < parabola_hint_range[1]:
                ...
                exp_diff_com_to_contact = self.box_pos[parabola_box_idx] + self.box_height * mm.unitY() / 2. - exp_com_pos
        ...
        if self.feet_contact_start_frame - 2 <= self.current_frame < self.feet_contact_start_frame:
            left_frame = self.feet_contact_start_frame - self.current_frame
        ...
        if self.feet_contact_start_frame <= self.current_frame:
            contact_body_height_diff.append(max(0., abs(
                self.skel.body('h_heel_left').to_world()[0] - self.box_pos[0][0]) - self.box_size[0] / 4.))
            ...
            contact_body_height_diff.append(max(0., abs(
                self.skel.body('h_heel_right').to_world()[0] - self.box_pos[0][0]) - self.box_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_heel_right').to_world()[1] - self.box_height - 0.0249)
            contact_body_height_diff.append(self.skel.body('h_heel_right').to_world()[2] - self.box_pos[0][2])
            contact_body_height_diff.append(
                self.skel.body('h_heel_left').to_world()[1] - self.skel.body('h_heel_right').to_world()[1])
    ...
    def step(self, _action):
        ...
        if self.feet_contact_end_frame >= self.current_frame:
            self.p_fc_hat[0] = 0
            self.p_fc_hat[1] = 0
