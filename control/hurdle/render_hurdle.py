...
import torch
from control.hurdle.hurdle_ppo_mp import PPO
...
def main():
    ...
    if not MOTION_ONLY:
        ppo.LoadModel('hurdle_model/' + 'max.pt')
        ppo.model.eval()
    ...
    # for bvh file
    bvh_qs = []
    bvh_file_name = 'hurdle_output.bvh'
    ...
    viewer.startTimer(1./18.)
    viewer.show()

    Fl.run()

    dart2bvh(bvh_file_name, ppo.env.skel, bvh_qs, 18)
