...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.world = pydart.World(1./180., cur_path+'/../../data/skel/human_mass_limited_dof.skel')
        self.world.control_skel = self.world.skeletons[1]
        self.skel = self.world.skeletons[1]
        self.Kp, self.Kd = 600., 49.

        self.ref_world = pydart.World(1./180., cur_path+'/../../data/skel/human_mass_limited_dof.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(cur_path + '/hurdle.skmo')
        self.ref_motion.reset_root_trajectory_hurdle(self.ref_skel)
        ...
        # draw box
        # add_box([name], [size], [color])
        self.box_height_list = [0.6, 0.01]
        self.box_height_mean = copy.deepcopy(self.box_height_list)
        self.box0_size = [3., self.box_height_list[0], 0.1]
        self.box1_size = [3.0, self.box_height_list[1], 0.3]
        self.world.skeletons[0].add_box("box0", self.box0_size, [0.8, 0.8, 0.8])
        self.world.skeletons[0].add_box("box1", self.box1_size, [0.8, 0.8, 0.8])

        self.box_pos = []
        self.box_pos.append(np.array([0., 0.5 * self.box_height_list[0], 2.8]))
        self.box_pos.append(np.array([0., 0.5 * self.box_height_list[1], 3.4]))
        ...
        # both hand
        # BOX CONTACT
        self.both_hand_contact_start_frame = 7
        self.both_hand_contact_end_frame = 17
    ...
    def state(self):
        ...
        box_p = np.array(
            [np.dot(R_pelvis.T, body.to_world([0, self.box_height_mean[box_idx], 0]) - p_pelvis) / 10. for box_idx, body
             in
    ...
    def reward(self):
        ...
        reward = r_p + r_v + r_fc + r_up + r_torque + r_root_ori

        self.rf_contact_end_frame1 = 7
        self.lf_contact_start_frame2 = 15
        self.lf_contact_end_frame2 = 16
        ...
        for parabola_hint_range in parabola_hint_ranges:
            ...
            if self.lf_contact_start_frame2 - 2 <= self.current_frame < self.lf_contact_start_frame2:
                ...
                exp_contact_body_height_diff.append(
                    self.skel.body('h_heel_left').to_world()[1] - self.box_height_list[
            ...
            if self.lf_contact_start_frame2 <= self.current_frame <= self.lf_contact_end_frame2:
                ...
                contact_body_height_diff.append(
                    self.skel.body('h_heel_left').to_world()[1] - self.box_height_list[1] - 0.0249)
    ...
    def is_done(self):
        ...
        if self.obstacle_collision:
            if PRINT_MODE:
                print("multiple body collision with a box occurs!")
            return True
        if self.is_foot_contact_same > 3:
