...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.world = pydart.World(1./250., cur_path+'/../../data/skel/human_mass_limited_dof_v2.skel')
        self.world.control_skel = self.world.skeletons[1]
        self.skel = self.world.skeletons[1]
        self.Kp, self.Kd = 1000., 60.

        self.ref_world = pydart.World(1./250., cur_path+'/../../data/skel/human_mass_limited_dof_v2.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(cur_path + '/high_jump.skmo')
        self.ref_motion.reset_root_trajectory_high_jump(self.ref_skel)
        self.ref_motion.refine_dqs(self.ref_skel)
        self.step_per_frame = 10

        self.height_hat_list = [1.8, 0.7]

        self.box_height_mean = copy.deepcopy(self.height_hat_list)

        # draw box
        # add box([name], [size], [color])
        self.box0_size = [5.0, self.height_hat_list[0], 0.1]
        self.box1_size = [5.0, self.height_hat_list[1], 2.5]

        self.world.skeletons[0].add_box("box0", self.box0_size, [0.9, 0.9, 0.9])
        self.world.skeletons[0].add_box("box1", self.box1_size, [0.75, 0.75, 0.75])

        self.box_pos = []
        self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[0], 3.0]))
        self.box_pos.append(np.array([0, 0.5 * self.height_hat_list[1], 4.3]))
        ...
        self.w_clear = 0.2
        self.exp_clear = 20.
        self.bar_contact_num = 0

        self.not_allow_collision_frame = 3
        ...
        self.openpose_head_list = []
        self.openpose_pelvis_list = []
        self.up_angle_list = []
        ...
        self.landing_start_frame = 115 - 78
        self.contact_label = np.load('../../data/contact_estimation/' + file_name + '_contact_info.npy')
        self.contact_label = self.contact_label[77:]

        self.no_vibe_pose_start_frame = 95 - 78
        self.no_vibe_pose_end_frame = 125 - 78

        self.bh_contact_start_frame = 120 - 78
        self.bh_contact_end_frame = 135 - 78

        self.jump_frame = -1

        self.landing_frame = self.bh_contact_start_frame

        for i in range(len(self.contact_label)):
            if i < self.landing_start_frame:
                if self.contact_label[i][1] == 1:
                    # print("left foot contact", i)
                    self.jump_frame = i
                if self.contact_label[i][0] == 1:
                    # print("right foot contact", i)
                    self.jump_frame = i
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            head_confidence = keypoint[0][1][2]
            midhip_confidence = keypoint[0][8][2]
            if head_confidence == 0:
                print("no head!!")
            if midhip_confidence == 0:
                print("no mid hip!!")
            head_pos = np.array([keypoint[0][1][0], keypoint[0][1][1]])
            midhip_pos = np.array([keypoint[0][8][0], keypoint[0][8][1]])
            self.pelvis_height2d_list.append(1080 - keypoint[0][8][1])

            self.openpose_head_list.append(head_pos)
            self.openpose_pelvis_list.append(midhip_pos)
    ...
    def state(self):
        ...
        box_p = np.array(
            [(body.to_world([0, 0.5 * self.box_height_mean[box_idx], 0]) - p_pelvis) / 10. for
             box_idx, body in enumerate(self.reward_boxes)]).flatten()
    ...
    def reward(self):
        ...
        if self.no_vibe_pose_start_frame <= self.current_frame <= self.no_vibe_pose_end_frame:
            self.w_p = 0.1
            self.w_v = 0.05
            self.w_fc = 0.4
            self.w_up = 0.35
        ...
        parabola_hint_ranges = [
            (self.jump_frame, self.landing_frame, ['h_hand_left', 'h_hand_right'], 1)]
        ...
        if self.bh_contact_start_frame - 2 <= self.current_frame < self.bh_contact_start_frame:
            left_frame = self.bh_contact_start_frame - self.current_frame
            exp_contact_body_height_diff.append(
                self.skel.body('h_hand_left').to_world()[1] - self.height_hat_list[1] - hand_margin - 0.1 * left_frame)
            exp_contact_body_height_diff.append(
                self.skel.body('h_hand_right').to_world()[1] - self.height_hat_list[1] - hand_margin - 0.1 * left_frame)
            exp_contact_body_height_diff.append(
                self.skel.body('h_hand_left').to_world()[1] - self.skel.body('h_hand_right').to_world()[1])
        ...
        self.bf_contact_start_frame = 48
        self.bf_contact_end_frame = 68
        if self.bf_contact_start_frame - 2 <= self.current_frame < self.bf_contact_end_frame:
            left_frame = self.bf_contact_start_frame - self.current_frame
            exp_contact_body_height_diff.append(
                self.skel.body('h_heel_left').to_world()[1] - self.height_hat_list[1] - 0.0249 - 0.1 * left_frame)
            exp_contact_body_height_diff.append(
        ...
        if self.bh_contact_start_frame <= self.current_frame <= self.bh_contact_end_frame:
            contact_body_height_diff.append(max(0., abs(
                self.skel.body('h_hand_left').to_world()[0] - self.box_pos[1][0]) - self.box1_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_hand_left').to_world()[1] - self.height_hat_list[1] - hand_margin)
            contact_body_height_diff.append(self.skel.body('h_hand_left').to_world()[2] - self.box_pos[1][2])
            contact_body_height_diff.append(max(0., abs(
                self.skel.body('h_hand_right').to_world()[0] - self.box_pos[1][0]) - self.box1_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_hand_right').to_world()[1] - self.height_hat_list[1] - hand_margin)
            contact_body_height_diff.append(self.skel.body('h_hand_right').to_world()[2] - self.box_pos[1][2])
            contact_body_height_diff.append(
                self.skel.body('h_hand_left').to_world()[1] - self.skel.body('h_hand_right').to_world()[1])

        if self.p_fc_hat[0] == 0:
            contact_body_height_diff.append(self.skel.body('h_heel_left').to_world()[1]-0.0249)
        if self.p_fc_hat[1] == 0:
            contact_body_height_diff.append(self.skel.body('h_heel_right').to_world()[1]-0.0249)

        if self.p_fc_hat[0] == 2:
            contact_body_height_diff.append(
                max(0., abs(self.skel.body('h_heel_left').to_world()[0] - self.box_pos[1][0]) - self.box1_size[0] / 4.))
            contact_body_height_diff.append(
        ...
        if self.p_fc_hat[1] == 2:
            contact_body_height_diff.append(max(0., abs(self.skel.body('h_heel_right').to_world()[0] - self.box_pos[1][0]) - self.box1_size[0]/4.))
            contact_body_height_diff.append(self.skel.body('h_heel_right').to_world()[1] - self.height_hat_list[1] - 0.0249)
    ...
    def is_done(self):
        if self.multi_body_collision:
            if PRINT_MODE: print("multiple body collision with a box occurs!")
            return True
        ...
        if self.up_angle_diff > 60. / 180. * pi:
            if PRINT_MODE:  print('up angle diff')
            return True
    ...
    def step(self, _action):
        ...
        for i in range(self.step_per_frame):
            if self.ext_force_duration > 0.:
                ...
                if self.ext_force_duration < 0.:
                    self.ext_force_duration = 0.
            torque = self.skel.get_spd_forces(q_des, self.Kp, self.Kd)
            self.step_torques.append(torque)
        ...
        if any(c_info[0] == 1 for c_info in self.contact_info):
            ...
            # todo: clearance reward
            if self.no_vibe_pose_start_frame + self.not_allow_collision_frame <= self.current_frame <= self.no_vibe_pose_end_frame:
                self.bar_contact_num += 1
        ...
        if self.jump_frame + self.not_allow_collision_frame < self.current_frame <= self.landing_frame:
            self.multi_body_collision = False
            self.obstacle_collision = False

        if self.landing_frame <= self.current_frame <= 57:
            if any(c_info[0] == 2 and c_info[1] in [7, 8, 9, 10, 11, 15] for c_info in self.contact_info):
                self.multi_body_collision = False
        ...
        if self.current_frame > self.landing_start_frame:
            if self.contact_label[self.current_frame][1] == 1:
                self.p_fc_hat[0] = 2
            if self.contact_label[self.current_frame][0] == 1:
                self.p_fc_hat[1] = 2
            if self.bh_contact_start_frame <= self.current_frame <= self.bh_contact_end_frame:
                self.p_fc_hat[2] = 2
                self.p_fc_hat[3] = 2
        ...
        if 8 < self.current_frame < 12:
            self.p_fc_hat[1] = 0

        if 47 < self.current_frame < 69:
            self.p_fc_hat[0] = 2
            self.p_fc_hat[1] = 2
    ...
    def reset(self):
        ...
        if self.current_frame == 0:
            ...
            temp_dq[3:6] = np.dot(init_rot.T, 3. * mm.unitZ())
