...
import torch
from control.jumptopout.jumptopout_ppo_mp import PPO
...
def main():
    ...
    if not MOTION_ONLY:
        ppo.LoadModel(f'{env_name}_model/max.pt')
        ppo.model.eval()
    ...
    if not MOTION_ONLY:
        ...
        viewer.doc.addRenderer('frame_label', yr.TextRenderer(rd_frame_text_label, rd_frame_text_label_pos, text_size=10))
        viewer.doc.addRenderer('frame_text', yr.TextRenderer(rd_frame_text, rd_frame_text_pos, text_size=10))
        viewer.doc.addRenderer('contact_label', yr.TextRenderer(rd_cotact_label_text, rd_cotact_label_text_pos, text_size=10))
        viewer.doc.addRenderer('contact_info', yr.TextRenderer(rd_contact_inf_text, rd_contact_inf_text_pos, text_size=10))
    ...
    def simulate_callback(frame):
        ...
        _q = ppo.env.ref_skel.positions()
        _q[:6] = ppo.env.skel.q[:6]
        ppo.env.ref_skel.set_positions(_q)
    ...
    viewer.startTimer(1./24.)
    viewer.show()

    Fl.run()

    dart2bvh(bvh_file_name, ppo.env.skel, bvh_qs, 24)
