...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.world = pydart.World(1./144., cur_path+'/../../data/skel/human_mass_limited_dof_v2.skel')
        self.world.control_skel = self.world.skeletons[1]
        self.skel = self.world.skeletons[1]
        self.Kp, self.Kd = 600., 49.

        self.ref_world = pydart.World(1./144., cur_path+'/../../data/skel/human_mass_limited_dof_v2.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(cur_path + '/lateral_jump.skmo')
        self.ref_motion.reset_root_trajectory_lateral_jump(self.ref_skel)
        ...
        self.world.skeletons[0].add_box("box0", self.box_size, [0.9, 0.9, 0.9])
        self.world.skeletons[0].add_box("box1", self.box_size, [0.9, 0.9, 0.9])

        self.world.skeletons[0].body("ground").set_collidable(False)

        self.box_pos = []
        self.box_pos.append(np.array([0., - 0.25, 0.]))
        self.box_pos.append(np.array([-1.5, -0.25, 0.]))
        ...
        self.idx_e = [self.skel.bodynode_index('h_heel_left'), self.skel.bodynode_index('h_heel_right')]
        ...
        self.lf_contact_end_frame1 = 22

        self.rf_contact_start_frame1 = 29
        self.rf_contact_end_frame1 = 84

        self.lf_contact_start_frame2 = 93
        self.lf_contact_end_frame2 = self.motion_len -1
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            head_pos = np.array([keypoint[0][0][0], keypoint[0][0][1]])
    ...
    def reward(self):
        ...
        if (self.current_frame <= self.lf_contact_end_frame1) or (
                self.rf_contact_start_frame1 - 2 <= self.current_frame <= self.rf_contact_end_frame1) or (
                self.lf_contact_start_frame2 - 2 <= self.current_frame):
        ...
        parabola_hint_ranges = [(self.lf_contact_end_frame1, self.rf_contact_start_frame1, ['h_heel_right'], 1),
                                (self.rf_contact_end_frame1, self.lf_contact_start_frame2, ['h_heel_left'], 0)]
        ...
        for parabola_hint_range in parabola_hint_ranges:
            ...
            if parabola_hint_range[0] < self.current_frame < parabola_hint_range[1]:
                ...
                exp_diff_com_to_contact = self.box_pos[parabola_box_idx] + self.box_size[1] * mm.unitY() / 2. - exp_com_pos
                ...
                self.ref_skel.set_positions(self.ref_motion.get_q(self.current_frame))
                self.ref_skel.set_velocities(self.ref_motion.get_dq(self.current_frame))
                # r_par = exp_reward_term(self.w_par, self.exp_par, np.asarray(pos_diff_ref_com_to_ref_contact) / len(pos_diff_ref_com_to_ref_contact))
        ...
        if self.rf_contact_start_frame1 - 2 <= self.current_frame < self.rf_contact_start_frame1:
            ...
            exp_contact_body_height_diff.append(
                self.skel.body('h_heel_right').to_world()[1] - self.box_height - 0.0249 - 0.1 * left_frame)
        ...
        if self.lf_contact_start_frame2 - 2 <= self.current_frame < self.lf_contact_start_frame2:
            ...
            exp_contact_body_height_diff.append(
                self.skel.body('h_heel_left').to_world()[1] - self.box_height - 0.0249 - 0.1 * left_frame)
        ...
        if self.lf_contact_start_frame1<= self.current_frame <= self.lf_contact_end_frame1:
            contact_body_height_diff.append(
                max(0., abs(self.skel.body('h_heel_left').to_world()[0] - self.box_pos[0][0]) - self.box_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_heel_left').to_world()[1] - self.box_height - 0.0249)
        ...
        if self.rf_contact_start_frame1 <= self.current_frame <= self.rf_contact_end_frame1:
            contact_body_height_diff.append(max(0.,
                                                ...
                                                self.box_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_heel_right').to_world()[1] - self.box_height - 0.0249)
            contact_body_height_diff.append(self.skel.body('h_heel_right').to_world()[2] - self.box_pos[1][2])

        if self.lf_contact_start_frame2 <= self.current_frame:
            contact_body_height_diff.append(
                max(0., abs(self.skel.body('h_heel_left').to_world()[0] - self.box_pos[0][0]) - self.box_size[0] / 4.))
            contact_body_height_diff.append(
                self.skel.body('h_heel_left').to_world()[1] - self.box_height - 0.0249)
    ...
    def is_done(self):
        ...
        if self.up_angle_diff > 45. / 180. * pi:
            if PRINT_MODE:
                print('up angle diff')
            return True
    ...
    def step(self, _action):
        ...
        if self.current_frame <= self.lf_contact_end_frame1:
            self.p_fc_hat[0] = 1
