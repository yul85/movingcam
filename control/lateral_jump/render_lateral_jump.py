...
import torch
from control.lateral_jump.lateral_jump_ppo_mp import PPO
...
def main():
    ...
    pydart.init()
    env_name = 'lateral_jump'
    ppo = PPO(env_name, 0, visualize_only=True)
    if not MOTION_ONLY:
        ppo.LoadModel('lateral_jump_model/' + 'max.pt')
        ppo.model.eval()
    ...
    # for bvh file
    bvh_qs = []
    bvh_file_name = 'ppo_lateral_jump_vibe.bvh'
