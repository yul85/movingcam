...
class SkateDartEnv(gym.Env):
    def __init__(self, env_config={}):
        ...
        self.world = pydart.World(1./150., self.base_dir + '/../../data/skel/human_mass_limited_dof_v2.skel')
        ...
        self.ref_world = pydart.World(1./150., self.base_dir + '/../../data/skel/human_mass_limited_dof_v2.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(self.base_dir + '/parkour4.skmo')
        for _i in range(len(self.ref_motion)):
            temp_q = np.asarray(self.ref_motion.get_q(_i))
            init_rot = mm.exp(temp_q[:3])
            temp_q[0:3] = mm.logSO3(np.dot(mm.exp(-0.05 * mm.unitX() + 0.65 * mm.unitY()), init_rot))
            temp_q[3:6] += np.array([0., 1.6, 0.1])
            self.ref_motion.qs[_i] = temp_q

        self.ref_motion.refine_dqs(self.ref_skel)
        self.step_per_frame = 5

        self.height_hat_list = [1.3, 1.5, 0.9, 0.9, 1.5, 1.5, 2.5]

        # draw box
        self.box_size = []
        self.box_size.append([2.0, self.height_hat_list[0], 0.3])
        self.box_size.append([2.0, self.height_hat_list[1], 0.3])
        self.box_size.append([2.0, self.height_hat_list[2], 0.3])
        self.box_size.append([2.0, self.height_hat_list[3], 0.3])
        self.box_size.append([2.0, self.height_hat_list[4], 0.3])
        self.box_size.append([2.0, self.height_hat_list[5], 5.0])
        self.box_size.append([2.0, self.height_hat_list[6], 0.3])
        ...
        self.box5_size = self.box_size[5]
        self.box6_size = self.box_size[6]
        ...
        self.world.skeletons[0].add_box("box5", self.box5_size, [1.0, 1.0, 1.0])
        self.world.skeletons[0].add_box("box6", self.box6_size, [0.6, 0.6, 0.6])

        # [2.3, 3.4, 5.0, 7.7, 10.0]
        self.box_pos = []
        self.box_pos.append(np.array([0., 0.5 * self.box0_size[1] + 1.0, -0.7]))
        self.box_pos.append(np.array([0., 0.5 * self.box1_size[1], 0.3]))
        self.box_pos.append(np.array([0., 0.5 * self.box2_size[1], 2.0]))
        self.box_pos.append(np.array([0., 0.5 * self.box3_size[1], 3.0]))
        self.box_pos.append(np.array([0., 0.5 * self.box4_size[1], 4.5]))
        self.box_pos.append(np.array([0., 0.5 * self.box5_size[1], 7.2]))
        self.box_pos.append(np.array([0., 0.5 * self.box6_size[1] + 1.6, 9.5]))

        self.box_rot = []
        self.box_rot.append(mm.logSO3(mm.rotX(mm.deg2Rad(110.))))
        self.box_rot.append(mm.logSO3(mm.rotY(mm.deg2Rad(0.))))
        self.box_rot.append(mm.logSO3(mm.rotY(mm.deg2Rad(0.))))
        self.box_rot.append(mm.logSO3(mm.rotY(mm.deg2Rad(0.))))
        self.box_rot.append(mm.logSO3(mm.rotY(mm.deg2Rad(0.))))
        self.box_rot.append(mm.logSO3(mm.rotY(mm.deg2Rad(0.))))
        self.box_rot.append(mm.logSO3(mm.rotX(mm.deg2Rad(10.))))
        ...
        self.reward_boxes = [body for body in self.world.skeletons[0].bodynodes[2:]]
        ...
        # box0
        self.rf_contact_start_frame1 = 0
        self.rf_contact_end_frame1 = 6

        # box1
        self.lf_contact_start_frame1 = 8
        self.lf_contact_end_frame1 = 16

        # box2
        self.rf_contact_start_frame2 = 26
        self.rf_contact_end_frame2 = 28

        # box3
        self.lf_contact_start_frame2 = 30
        self.lf_contact_end_frame2 = 34

        # box4
        self.both_f_contact_start_frame1 = 46
        self.both_f_contact_end_frame1 = 51

        # box5
        self.rf_contact_start_frame3 = 69
        self.rf_contact_end_frame3 = 74

        self.lf_contact_start_frame3 = 72
        self.lf_contact_end_frame3 = 80

        self.lh_contact_start_frame1 = 79
        self.lh_contact_end_frame1 = 90

        self.rh_contact_start_frame1 = 86
        self.rh_contact_end_frame1 = 91

        self.both_f_contact_start_frame2 = 97
        self.both_f_contact_end_frame2 = 100

        # box6
        self.both_h_contact_start_frame1 = 109
        self.both_h_contact_end_frame1 = 114

        # box5
        self.both_f_contact_start_frame3 = 124
        self.both_f_contact_end_frame3 = 135

        self.rf_contact_start_frame4 = 136
        self.rf_contact_end_frame4 = 139

        self.rh_contact_start_frame2 = 136
        self.rh_contact_end_frame2 = 154

        self.lh_contact_start_frame2 = 148
        self.lh_contact_end_frame2 = 154

        # ground
        self.rf_contact_start_frame5 = 156
        self.rf_contact_end_frame5 = 156
        ...
        if np.isnan(self.up_angle_list[0]):
            self.up_angle_list[0] = 0.
        if np.isnan(self.up_angle_list[-1]):
            self.up_angle_list[-1] = 0.

        prev_up_angle = self.up_angle_list[0]
        prev_up_angle_idx = 0
        for up_angle_idx, up_angle in enumerate(self.up_angle_list):
            if not np.isnan(up_angle):
                if up_angle_idx - prev_up_angle_idx >= 2:
                    for fix_up_angle_idx in range(prev_up_angle_idx+1, up_angle_idx):
                        ratio = (fix_up_angle_idx - prev_up_angle_idx) / (up_angle_idx - prev_up_angle_idx)
                        self.up_angle_list[fix_up_angle_idx] = (1. - ratio) * prev_up_angle + ratio * up_angle
                prev_up_angle = up_angle
                prev_up_angle_idx = up_angle_idx
    ...
    def state(self):
        ...
        box_p = np.array([np.dot(R_pelvis.T, body.to_world([0, 0.5 * self.box_size[box_idx+1][1], 0]) - p_pelvis)/10. for box_idx, body in
    ...
    def reward(self):
        ...
        pelvis_ori_diff = []
        pelvis_hint_ranges = [
            (self.rf_contact_start_frame1, self.rf_contact_end_frame1, 0),
            (self.lf_contact_start_frame1, self.lf_contact_end_frame1, 1),
            (self.rf_contact_start_frame2, self.rf_contact_end_frame2, 2),
            (self.lf_contact_start_frame2, self.lf_contact_end_frame2, 3),
            (self.both_f_contact_start_frame1, self.both_f_contact_end_frame1, 4),
            (self.both_f_contact_start_frame2, self.both_f_contact_end_frame2, 5),
            (self.both_h_contact_start_frame1, self.both_h_contact_end_frame1, 6),
        ]
        ...
        parabola_hint_ranges = [
            (self.lf_contact_end_frame1, self.rf_contact_start_frame2, ['h_heel_right'], 2),
            (self.lf_contact_end_frame2, self.both_f_contact_start_frame1, ['h_heel_left', 'h_heel_right'], 4),
            (self.both_f_contact_end_frame1, self.rf_contact_start_frame3, ['h_heel_right'], 5, -1. * mm.unitZ()),
        ]
        for parabola_hint_range in parabola_hint_ranges:
            ...
            if parabola_hint_range[0] < self.current_frame < parabola_hint_range[1]:
                ...
                if len(parabola_hint_range) == 5:
                    exp_diff_com_to_contact += parabola_hint_range[4]
        ...
        exp_height_hint_ranges = [
            (self.lf_contact_start_frame1, ['h_heel_left'], 1),
            (self.rf_contact_start_frame2, ['h_heel_right'], 2),
            (self.lf_contact_start_frame1, ['h_heel_left'], 3),
            (self.both_f_contact_start_frame1, ['h_heel_left', 'h_heel_right'], 4),
            (self.rf_contact_start_frame3, ['h_heel_right'], 5),
            (self.lf_contact_start_frame3, ['h_heel_left'], 5),
            (self.lh_contact_start_frame1, ['h_hand_left'], 5),
            (self.rh_contact_start_frame1, ['h_hand_right'], 5),
            (self.both_f_contact_start_frame2, ['h_heel_left', 'h_heel_right'], 5),
            (self.both_f_contact_start_frame3, ['h_heel_left', 'h_heel_right'], 5),
            (self.rf_contact_start_frame4, ['h_heel_right'], 5),
            (self.rh_contact_start_frame2, ['h_hand_right'], 5),
            (self.lh_contact_start_frame2, ['h_hand_left'], 5),
            (self.rf_contact_start_frame3, ['h_heel_right'], -1),
        ]
        for exp_height_hint_range in exp_height_hint_ranges:
            height_box_idx = exp_height_hint_range[2]
            if exp_height_hint_range[0] - 2 <= self.current_frame < exp_height_hint_range[0]:
                left_frame = exp_height_hint_range[0] - self.current_frame
                for height_hint_body_name in exp_height_hint_range[1]:
                    if height_box_idx == -1:
                        exp_contact_body_height_diff.append(get_lowest_body_point(self.skel.body(height_hint_body_name))[1] - 0.1 * left_frame)
                    else:
                        local_pos = to_local(self.world.skeletons[0].body(height_box_idx+1), get_lowest_body_point(self.skel.body(height_hint_body_name)))
                        exp_contact_body_height_diff.append(local_pos[1] - self.box_size[height_box_idx][1]/2. - 0.1 * left_frame)
                if len(exp_height_hint_range[1]) == 2:
                    exp_contact_body_height_diff.append(get_lowest_body_point(self.skel.body(exp_height_hint_range[1][0]))[1] - get_lowest_body_point(self.skel.body(exp_height_hint_range[1][1]))[1])
        ...
        if self.lf_contact_start_frame1 <= self.current_frame <= self.lf_contact_end_frame1:
            contact_body_height_diff.append(get_lowest_body_point(self.skel.body('h_heel_left'))[1] + lowest_contact_margin)

        if self.rf_contact_start_frame1 <= self.current_frame <= self.rf_contact_end_frame1:
            contact_body_height_diff.append(get_lowest_body_point(self.skel.body('h_heel_right'))[1] + lowest_contact_margin)

        height_hint_ranges = [
            (self.lf_contact_start_frame1, self.lf_contact_end_frame1, ['h_heel_left'], 1, False),  # boolean: target zone or just height
            (self.rf_contact_start_frame2, self.rf_contact_end_frame2, ['h_heel_right'], 2, True),
            (self.lf_contact_start_frame2, self.lf_contact_end_frame2, ['h_heel_left'], 3, True),
            (self.both_f_contact_start_frame1, self.both_f_contact_end_frame1, ['h_heel_left', 'h_heel_right'], 4, True),

            (self.rf_contact_start_frame3, self.rf_contact_end_frame3, ['h_heel_right'], 5, False),
            (self.lf_contact_start_frame3, self.lf_contact_end_frame3, ['h_heel_left'], 5, False),
            (self.lh_contact_start_frame1, self.lh_contact_end_frame1, ['h_hand_left'], 5, False),

            (self.rh_contact_start_frame1, self.rh_contact_end_frame1, ['h_hand_right'], 5, False),
            (self.both_f_contact_start_frame2, self.both_f_contact_end_frame2, ['h_heel_left', 'h_heel_right'], 5, False),
            (self.both_f_contact_start_frame3, self.both_f_contact_end_frame3, ['h_heel_left', 'h_heel_right'], 5, False),
            (self.rf_contact_start_frame4, self.rf_contact_end_frame4, ['h_heel_right'], 5, False),
            (self.rh_contact_start_frame2, self.rh_contact_end_frame2, ['h_hand_left'], 5, False),
            (self.lh_contact_start_frame2, self.lh_contact_end_frame2, ['h_hand_right'], 5, False),
            (self.rf_contact_start_frame5, self.rf_contact_end_frame5, ['h_heel_left'], -1, False),
        ]
                ...
                for height_hint_body_name in height_hint_range[2]:
                    if height_box_idx == -1:
                        if 'hand' in height_hint_body_name:
                            contact_body_height_diff.append(self.skel.body(height_hint_body_name).to_world()[1] - hand_margin)
                        elif 'heel' in height_hint_body_name:
                            contact_body_height_diff.append(self.skel.body(height_hint_body_name).to_world()[1] - foot_margin)
                    else:
                        local_pos = to_local(self.world.skeletons[0].body(height_box_idx+1), self.skel.body(height_hint_body_name).to_world())
                        if height_hint_range[4]:
    ...
    def step(self, _action):
        ...
        if self.rh_contact_start_frame1 <= self.current_frame <= self.rh_contact_end_frame1:
            self.p_fc_hat[3] = 6
        ...
        if self.both_f_contact_start_frame3 <= self.current_frame <= self.both_f_contact_end_frame3:
            self.p_fc_hat[0] = 6
            self.p_fc_hat[1] = 6

        if self.rf_contact_start_frame4 <= self.current_frame <= self.rf_contact_end_frame4:
            self.p_fc_hat[1] = 6

        if self.rh_contact_start_frame2 <= self.current_frame <= self.rh_contact_end_frame2:
            self.p_fc_hat[3] = 6

        if self.lh_contact_start_frame2 <= self.current_frame <= self.lh_contact_end_frame2:
            self.p_fc_hat[2] = 6

        if self.rf_contact_start_frame5 <= self.current_frame <= self.rf_contact_end_frame5:
            self.p_fc_hat[1] = 0
        ...
        if len(self.is_foot_contact_same_list) < 30:
            self.foot_contact_violation = sum(self.is_foot_contact_same_list) > 5
        else:
            self.foot_contact_violation = sum(self.is_foot_contact_same_list[-30:]) > 5
    ...
    def reset(self):
        ...
        self.skel.set_velocities(np.zeros_like(self.ref_motion.get_dq(self.current_frame)))
        self.count_frame = 0
        self.foot_contact_violation = False
        self.contact_violation_num = 0
        ...
        box_q[39:42] = self.box_pos[5]
        box_q[45:48] = self.box_pos[6]

        box_q[6:9] = self.box_rot[0]
        box_q[12:15] = self.box_rot[1]
        box_q[18:21] = self.box_rot[2]
        box_q[24:27] = self.box_rot[3]
        box_q[30:33] = self.box_rot[4]
        box_q[36:39] = self.box_rot[5]
        box_q[42:45] = self.box_rot[6]
