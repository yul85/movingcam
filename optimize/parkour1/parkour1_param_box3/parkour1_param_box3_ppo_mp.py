...
from optimize.parkour1.parkour1_param_box3.parkour1_param_box3_env import SkateDartEnv
from optimize.parkour1.parkour1_param_box2.parkour1_param_box2_ppo_mp import PPO as Win2PPO
...
def worker(rnn_len, proc_num, state_sender, result_sender, action_receiver, reset_receiver, motion_receiver, init_distribution_receiver):
    """
    :type rnn_len: int
    :type proc_num: int
    ...
    while True:
        ...
        if reset_flag in [1, 2]:
            if reset_flag == 2:
                env.hard_reset()
            q, dq, box_pos = init_distribution_receiver.recv()

            state = env.reset_init_distribution(q, dq, box_pos)
...
class PPO(object):
    def __init__(self, env_name, num_slaves=1, eval_print=True, eval_log=True, visualize_only=False):
        ...
        self.init_distribution_sender = []  # type: List[Connection]
        self.envs = []  # type: List[Process]

        self.init_envs()

        self.make_init_distribution_ppo = Win2PPO(env_name, 0, visualize_only=True)
        self.make_init_distribution_ppo.LoadModel('../parkour1_param_box2/parkour1_param_box2_model/max.pt')
        self.make_init_distribution_ppo.model.eval()
        init_q, init_dq, init_box_pos0, init_box_pos1 = self.make_init_distribution_ppo.init_distribution_generate()
        self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos0, init_box_pos1)
    ...
    def init_envs(self):
        for slave_idx in range(self.num_slaves):
            ...
            init_distribution_s, init_distribution_r = Pipe()
            p = Process(target=worker, args=(-1, slave_idx, s_s, r_s, a_r, reset_r, motion_r, init_distribution_r))
            ...
            self.init_distribution_sender.append(init_distribution_s)
            self.envs.append(p)
            p.start()
    ...
    def reinit_env(self, slave_idx):
        ...
        self.init_distribution_sender[slave_idx] = init_distribution_s
    ...
    def init_distribution_generate(self):
        init_q, init_dq, init_box_pos0, init_box_pos1 = self.make_init_distribution_ppo.init_distribution_generate()
        state = self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos0, init_box_pos1)
        while True:
            action_dist, _ = self.make_init_distribution_ppo.model(torch.tensor(state.reshape(1, -1)).float())
            action = action_dist.loc.detach().numpy()
            state, reward, is_done, _ = self.make_init_distribution_ppo.env.step(action[0])
            if not is_done and self.make_init_distribution_ppo.env.current_frame == self.make_init_distribution_ppo.env.rf_contact_start_frame2:
                break
            elif is_done:
                init_q, init_dq, init_box_pos0, init_box_pos1 = self.make_init_distribution_ppo.init_distribution_generate()
                state = self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos0, init_box_pos1)

        q = self.make_init_distribution_ppo.env.skel.q
        dq = self.make_init_distribution_ppo.env.skel.dq
        box_pos = self.make_init_distribution_ppo.env.target_box_pos
        return [q, dq, box_pos]
    ...
    def envs_resets(self, reset_flag):
        for i in range(len(self.reset_sender)):
            ...
            if reset_flag in [1, 2]:
                self.init_distribution_sender[i].send(self.init_distribution_generate())
