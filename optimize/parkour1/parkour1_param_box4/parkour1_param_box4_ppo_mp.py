...
from optimize.parkour1.parkour1_param_box4.parkour1_param_box4_env import SkateDartEnv
from optimize.parkour1.parkour1_param_box3.parkour1_param_box3_ppo_mp import PPO as Win3PPO
...
class PPO(object):
    def __init__(self, env_name, num_slaves=1, eval_print=True, eval_log=True, visualize_only=False):
        ...
        self.make_init_distribution_ppo = Win3PPO(env_name, 0, visualize_only=True)
        self.make_init_distribution_ppo.LoadModel('../parkour1_param_box3/parkour1_param_box3_model/max.pt')
        self.make_init_distribution_ppo.model.eval()
        init_q, init_dq, init_box_pos = self.make_init_distribution_ppo.init_distribution_generate()
        self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos)
    ...
    def init_distribution_generate(self):
        init_q, init_dq, init_box_pos = self.make_init_distribution_ppo.init_distribution_generate()
        state = self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos)
        while True:
            ...
            if not is_done and self.make_init_distribution_ppo.env.current_frame == self.make_init_distribution_ppo.env.both_f_contact_start_frame1:
                break
            elif is_done:
                init_q, init_dq, init_box_pos = self.make_init_distribution_ppo.init_distribution_generate()
                state = self.make_init_distribution_ppo.env.reset_init_distribution(init_q, init_dq, init_box_pos)
