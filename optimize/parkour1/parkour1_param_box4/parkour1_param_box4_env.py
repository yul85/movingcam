...
class SkateDartEnv(gym.Env):
    ...
    def is_done(self):
        if self.current_frame == self.both_f_contact_end_frame2:
            if PRINT_MODE:
                print("Window end!!")
            return True
    ...
    def reset_optimized_box(self, init_q, init_dq, box_pos_list):
        ...
        self.ref_motion.reset_root_trajectory_parkour1(self.ref_skel)
        self.ref_motion.refine_dqs(self.ref_skel)
        self.continue_from_frame(self.both_f_contact_start_frame1)
        ...
        box_q[27:30] = box_pos_list[3]
        box_q[33:36] = box_pos_list[4]
    ...
    def reset_init_distribution(self, init_q, init_dq, init_box_pos):
        ...
        self.box_pos[3] = init_box_pos

        init_target_box_pos = np.array(self.init_target_box_pos[4])
        init_target_pos_x = init_target_box_pos[2]
        init_target_pos_y = init_target_box_pos[1]
        distance_from_prev_box = init_target_pos_x - self.box_pos[3][2]
        ...
        target_box_pos = np.array([self.box_pos[4][0], y_val, z_val])
