...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            if len(keypoint) == 0:
                # print("NO OPENPOSE RESULT!!", json_file, file=sys.stderr)
    ...
    def reset_optimized_box(self, init_q, init_dq, box_pos_list):
        ...
        ######################
        init_target_box_pos = np.array(self.init_target_box_pos[2])
        ######################

        box_q = self.world.skeletons[0].q
        box_q[9:12] = box_pos_list[0]
        box_q[15:18] = box_pos_list[1]
        box_q[21:24] = box_pos_list[2]
        self.target_box_diff = init_target_box_pos - np.array(box_q[21:24])
    ...
    def reset_init_distribution(self, init_q, init_dq, init_box_pos0, init_box_pos1):
        self.world.reset()
        self.ref_motion.reset_root_trajectory_parkour1(self.ref_skel)
        self.ref_motion.refine_dqs(self.ref_skel)
        self.continue_from_frame(self.lf_contact_start_frame2)
        ...
        self.box_pos[0] = init_box_pos0
        self.box_pos[1] = init_box_pos1

        init_target_box_pos = np.array(self.init_target_box_pos[2])
        init_target_pos_x = init_target_box_pos[2]
        init_target_pos_y = init_target_box_pos[1]
        distance_from_prev_box = init_target_pos_x - self.box_pos[1][2]
        distance_margin = 0.1 * distance_from_prev_box
        init_target_height = init_target_box_pos[1] * 2.
        h_margin = 0.1 * init_target_height
        ...
        target_box_pos = np.array([self.box_pos[2][0], y_val, z_val])
        ...
        box_q[21:24] = target_box_pos
        self.target_box_diff = init_target_box_pos - np.array(box_q[21:24])
        box_q[27:30] = self.box_pos[3] - self.target_box_diff
        box_q[33:36] = self.box_pos[4] - self.target_box_diff
        self.world.skeletons[0].set_positions(box_q)

        for i in range(len(self.height_hat_list)):
            self.height_hat_list[i] = 0.5 * self.box_size[i][1] + self.box_pos[i][1]
