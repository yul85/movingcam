...
from optimize.parkour1.parkour1_param_box2.parkour1_param_box2_env import SkateDartEnv
from optimize.parkour1.parkour1_param_box1.parkour1_param_box1_ppo_mp import PPO as Win1PPO
...
def worker(rnn_len, proc_num, state_sender, result_sender, action_receiver, reset_receiver, motion_receiver, init_distribution_receiver):
    ...
    while True:
        ...
        if reset_flag in [1, 2]:
            if reset_flag == 2:
                env.hard_reset()
            q, dq, box_pos0, box_pos1 = init_distribution_receiver.recv()

            state = env.reset_init_distribution(q, dq, box_pos0, box_pos1 )
...
class PPO(object):
    def __init__(self, env_name, num_slaves=1, eval_print=True, eval_log=True, visualize_only=False):
        ...
        self.make_init_distribution_ppo = Win1PPO(env_name, 0, visualize_only=True)
        self.make_init_distribution_ppo.LoadModel('../parkour1_param_box1/parkour1_param_box1_model/max.pt')
        self.make_init_distribution_ppo.model.eval()
        self.make_init_distribution_ppo.env.reset()
    ...
    def init_distribution_generate(self):
        state = self.make_init_distribution_ppo.env.reset()
        while True:
            ...
            if not is_done and self.make_init_distribution_ppo.env.current_frame == self.make_init_distribution_ppo.env.lf_contact_start_frame2:
                break
            elif is_done:
                state = self.make_init_distribution_ppo.env.reset()
        ...
        box_pos0 = self.make_init_distribution_ppo.env.target_box_pos
        box_pos1 = self.make_init_distribution_ppo.env.target_box_pos1
        return [q, dq, box_pos0, box_pos1]
...
if __name__ == "__main__":
    ...
    tic = time.time()
    if len(sys.argv) < 2:
        ppo = PPO('parkour1_param_box2', 16)
    else:
