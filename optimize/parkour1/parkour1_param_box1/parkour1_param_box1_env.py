...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.box_size = [self.box0_size, self.box1_size, self.box2_size, self.box3_size, self.box4_size]
        ...
        self.box_pos.append(np.array([0., 0.5 * self.height_hat_list[4], 8.2]))
    ...
    def state(self):
        ...
        box_p = np.array([(body.to_world([0, 0.5*self.box_height_mean[box_idx], 0]) - p_pelvis)/10. for box_idx, body in
    ...
    def reward(self):
        ...
        if (self.lf_contact_start_frame1 - 2 <= self.current_frame <= self.lf_contact_end_frame1) \
                or (self.rf_contact_start_frame1 - 2 <= self.current_frame <= self.lf_contact_end_frame2) \
    ...
    def is_done(self):
        if self.current_frame == self.rf_contact_end_frame2:
            if PRINT_MODE:
                print("Window end!!")
            return True
    ...
    def step(self, _action):
        ...
        if self.rf_contact_end_frame1 + 4 < self.current_frame <= self.lf_contact_start_frame2:
            self.multi_body_collision = False
            self.obstacle_collision = False
    ...
    def reset(self):
        ...
        init_target_box_pos = np.array(self.init_target_box_pos[0])
        init_target_pos_x = init_target_box_pos[2]
        init_target_pos_y = init_target_box_pos[1]
        distance_from_prev_box = init_target_pos_x
        ...
        target_box_pos = np.array([self.box_pos[0][0], y_val, z_val])

        self.target_box_pos = target_box_pos

        init_target_box_pos1 = np.array(self.init_target_box_pos[1])
        init_target_pos1_x = init_target_box_pos1[2]
        init_target_pos1_y = init_target_box_pos1[1]
        distance1_from_prev_box = abs(init_target_pos1_x - self.box_pos[0][2])
        distance1_margin = 0.2 * distance1_from_prev_box
        init_target_height1 = init_target_box_pos1[1] * 2.
        h1_margin = 0.2 * init_target_height1
        z_val1 = uniform(init_target_pos1_x - distance1_margin, init_target_pos1_x + distance1_margin)
        y_val1 = uniform(init_target_pos1_y - h1_margin, init_target_pos1_y + h1_margin)
        target_box_pos1 = np.array([self.box_pos[1][0], y_val1, z_val1])

        self.target_box_pos1 = target_box_pos1

        box_q = self.world.skeletons[0].q
        box_q[9:12] = target_box_pos
        box_q[15:18] = target_box_pos1
        self.target_box_diff = init_target_box_pos1 - np.array(box_q[15:18])
        box_q[21:24] = self.box_pos[2] - self.target_box_diff
        box_q[27:30] = self.box_pos[3] - self.target_box_diff
        box_q[33:36] = self.box_pos[4] - self.target_box_diff
    ...
    def reset_optimized_box(self, init_q, init_dq, box_pos_list):
        ...
        ######################
        init_target_box_pos = np.array(self.init_target_box_pos[0])
        target_box_pos = box_pos_list[0]
        init_target_box_pos1 = np.array(self.init_target_box_pos[1])
        target_box_pos1 = box_pos_list[1]
        ######################
