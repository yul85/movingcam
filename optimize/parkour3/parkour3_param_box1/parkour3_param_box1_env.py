...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        self.box3_size = [3.0, self.height_hat_list[3], 0.1]
    ...
    def is_done(self):
        if self.current_frame == self.lf_contact_end_frame2:
            if PRINT_MODE:
                print("Window end!!")
            return True
    ...
    def reset(self):
        ...
        init_target_box_pos = np.array(self.init_target_box_pos[1])
        init_target_pos_x = init_target_box_pos[2]
        init_target_pos_y = init_target_box_pos[1]
        distance_from_prev_box = init_target_pos_x - self.box_pos[0][2]
        ...
        target_box_pos = np.array([self.box_pos[1][0], y_val, z_val])
        ...
        box_q[15:18] = target_box_pos
        self.target_box_diff = init_target_box_pos - np.array(box_q[15:18])
    ...
    def reset_init_distribution(self, init_q, init_dq, init_box_pos):
        ...
        self.box_pos[1] = init_box_pos
        self.height_hat_list[1] = init_box_pos[1]+0.5*self.box_height_mean[1]
        ...
        box_q = self.world.skeletons[0].q
        box_q[9:12] = self.box_pos[0]
        self.target_box_diff = self.init_target_box_pos[0] - np.array(box_q[9:12])
        box_q[15:18] = self.box_pos[1] - self.target_box_diff
        box_q[21:24] = self.box_pos[2] - self.target_box_diff
        box_q[27:30] = self.box_pos[3] - self.target_box_diff
        self.world.skeletons[0].set_positions(box_q)
